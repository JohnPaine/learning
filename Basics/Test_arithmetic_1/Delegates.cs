...
namespace TestArithmetic
{
    class Delegates
    {
        ...
        static void HardToil(ProgressReporter pr) {
            for (var i = 0; i < 10; ++i) {
        ...
        static void WriteProgressToConsole(int percent) {
            Console.WriteLine("Work completion - {0}", percent);
        }

        static void WriteProgressToFile(int percent) {
            System.IO.File.AppendAllText("progress.txt", "Work completion - " + percent.ToString(CultureInfo.InvariantCulture) + "\n");
        }
